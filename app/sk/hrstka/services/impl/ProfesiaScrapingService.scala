...
class ProfesiaScrapingService @Inject() (compRepository: CompRepository,
  ...
  override def scrape: Future[ScrapingResult] = compRepository.all().flatMap { dbComps =>
    techRepository.all().flatMap { dbTechs =>
      val scrapeAllPostingUrlsFuture = Future {
        // Scrape posting URLs
        scrapeAllPostingUrls(1, Nil)
      }(scraperContext)

      scrapeAllPostingUrlsFuture.flatMap { postingUrls =>
        val scrapedCompFutures = postingUrls.map { postingUrl =>
          val profesiaStaticCompScraperFuture = Future {
            ProfesiaStaticCompScraper(postingUrl)
          }(scraperContext)

          profesiaStaticCompScraperFuture.map { staticCompScraperResult =>
        ...
        Future.sequence(scrapedCompFutures).map { scrapedComps =>
...
private object ProfesiaScrapingService {
  ...
  lazy val scraperContext: ExecutionContext = Akka.system.dispatchers.lookup("akka.actor.scraper-context")
