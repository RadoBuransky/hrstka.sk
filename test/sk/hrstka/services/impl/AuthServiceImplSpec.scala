...
import sk.hrstka.models.domain.{Email, UserSpec}
...
class AuthServiceImplSpec extends BaseSpec {
  ...
  it should "find an user in DB and map it to domain" in new TestScope {
    ...
    // Execute
    assert(authService.findByEmail(Email(rado.email)).futureValue.contains(UserSpec.rado))
  }
  ...
  it should "return the user if exists and the password is correct" in new TestScope {
    ...
    // Execute
    assert(authService.authenticate(Email(rado.email), radoPassword).futureValue.contains(hrstka.models.domain.UserFactory(rado)))
  }
  ...
  it should "return none if user exists but the password is wrong" in new TestScope {
    ...
    // Execute
    assert(authService.authenticate(Email(rado.email), "123").futureValue.isEmpty)
  }
  ...
  it should "return none if user does not exist" in new TestScope {
    ...
    // Execute
    assert(authService.authenticate(Email(rado.email), radoPassword).futureValue.isEmpty)
  }
